'use strict';
// Effort capture/inheritance test suite. Run: node --test
...
// NOTA: casos compartilham estado (cache/sessions/mtime no fs real). node --test
// roda top-level tests em ordem de definição — NÃO paralelizar/concurrency aqui.
...
test('picker-ultracode', () => {
  const t1 = writeT('s1.jsonl',
    ...
    500);
  assert.strictEqual(parseTranscript(t1, 's1', claudeDir).lastEffort, 'ultracode');
});
...
test('clear-born-inherits', () => {
  ...
  assert.strictEqual(rB.lastEffort, 'max', 'clear-born-inherits');
  // state cache persisted for sB?
  const stB = JSON.parse(fs.readFileSync(path.join(claudeDir, 'cache', 'statusline-state-sB.json'), 'utf8'));
  assert.strictEqual(String(stB.v), '2', 'cache-schema-v2');
  assert.strictEqual(String(stB.state.clearBorn), 'true', 'cache-clearBorn-sticky');
  assert.strictEqual(stB.state.lastEffort, 'max', 'cache-effort-persisted');
});

// C: clear-born after B. B has no /effort line → must fall back to B's state cache.
test('chained-clear-inherits', () => {
  ...
  fs.utimesSync(path.join(proj, 'sB.jsonl'), tBnow, tBnow);
  assert.strictEqual(parseTranscript(tC, 'sC', claudeDir).lastEffort, 'max');
});

// ── Case 6: incremental render — second parse of sB uses cache, effort stays
test('second-render-cached', () => {
  assert.strictEqual(parseTranscript(path.join(proj, 'sB.jsonl'), 'sB', claudeDir).lastEffort, 'max');
});

// ── Case 7: clear-born then user changes effort → own /effort wins over inheritance
test('own-effort-beats-inherited', () => {
  const tD = writeT('sD.jsonl',
    ...
    50);
  assert.strictEqual(parseTranscript(tD, 'sD', claudeDir).lastEffort, 'low');
});

// ── Case 8: turn counting skips command/meta lines
test('turns-skip-commands', () => {
  ...
  assert.strictEqual(String(r8.turnCount), '1', 'turns-skip-commands');
  assert.strictEqual(r8.lastUserMessage, 'real question', 'lastmsg-no-xml');
});
